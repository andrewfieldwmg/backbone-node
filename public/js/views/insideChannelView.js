		  ...
		  if (channel.currentStreamStatus === 'started') {
		      ...
		      console.log('there is an active stream');
			   ...
			   $.when(
			      ...
			      $('#start-recording').addClass('disabled'), $('#start-file-stream').addClass('disabled')
				   ...
				   localStorage.setItem("userRole", "listener");
				    ...
				    localStorage.setItem('currentStreamId', channel.currentStreamId);
				    localStorage.setItem('currentStreamerUserId', channel.currentStreamerId);
				  ...
				  new AudioPlayerView({
					...
					audioController.playMp3Stream(socket);
				    }
